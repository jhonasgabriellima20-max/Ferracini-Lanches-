...
async function findListedBlob(pathname){
  const listed = await listBlobs({ prefix: pathname, limit: 100 });
...
async function getBlob(pathname, access){
  ...
  // For private stores, resolving the blob URL first is more reliable than
  // issuing a raw fetch against downloadUrl. The Blob SDK handles the data
  // plane authentication/signing when it receives the canonical blob URL.
  const blob = await findListedBlob(pathname);
  if(!blob) return null;
  const target = blob.url || blob.downloadUrl;
  if(!target) return null;

  try{
    return await callWithAuth(auth =>
      get(target, { access, useCache: false, ...auth })
    );
  }catch(err){
    if(isNotFound(err)) return null;
    throw err;
