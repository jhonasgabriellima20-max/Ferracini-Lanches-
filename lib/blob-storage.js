...
async function fetchListedBlob(pathname){
  ...
  const token = String(process.env.BLOB_READ_WRITE_TOKEN || '').trim();
  const oidcToken = await runtimeOidcToken();
  const bearers = [...new Set([oidcToken, token, ''])];
  let lastAuthError;
  ...
  for(const bearer of bearers){
    ...
    if(response.status === 401 || response.status === 403){
      const error = new Error(`Vercel Blob direct fetch: ${response.status} ${response.statusText}`);
  ...
  throw lastAuthError || new Error('blob_direct_fetch_failed');
}
...
async function getBlob(pathname, access){
  ...
  try{
    ...
    if(isAuthError(err)){
      return fetchListedBlob(pathname);
    }
    throw err;
  }
}
