...
async function getViaPresignedUrl(pathname, access){
  if(access !== 'private') return null;
  const { issueSignedToken, presignUrl } = await import('@vercel/blob');
  const validUntil = Date.now() + (5 * 60 * 1000);

  return callWithAuth(async auth => {
    ...
    if(!response.ok){
      const error = new Error(`Vercel Blob presigned fetch: ${response.status} ${response.statusText}`);
...
async function getBlob(pathname, access){
  ...
  return getViaPresignedUrl(pathname, access);
}
