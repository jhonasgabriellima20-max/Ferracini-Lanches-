...
function contarLanches(itens){
  let quantidade = 0;
  for(const item of Array.isArray(itens) ? itens : []){
    const qtd = clamp(Math.trunc(Number(item?.quantidade) || 0), 0, 50);
    if(qtd <= 0) continue;
    const nome = String(item?.nome || '');
    if(tempoPadrao(nome) <= 0) continue;
    quantidade += qtd;
  }
  return quantidade;
}

function calcularTempoInternoLanches(itens){
  return contarLanches(itens) * MINUTOS_POR_LANCHE;
}

function tempoInternoDoPedido(pedido){
  const salvo = Number(pedido?.atendimento?.tempoInternoMinutos);
  if(Number.isFinite(salvo) && salvo >= 0) return Math.ceil(salvo);
  return calcularTempoInternoLanches(pedido?.itens);
}

function calcularFilaTempoInterno(pedidos, agora = new Date()){
  ...
  for(const entry of ordenados){
    if(ultimoTs !== null){
      ...
      minutosPendentes = Math.max(0, minutosPendentes - minutosPassados);
    }

    const tempoPedido = tempoInternoDoPedido(entry.pedido);
    if(tempoPedido > 0){
      minutosPendentes += tempoPedido;
  ...
  return {
    filaMinutos: Math.ceil(minutosPendentes),
    pedidosConsiderados,
  };
}
